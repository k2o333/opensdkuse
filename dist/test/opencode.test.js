...
import { connectOrStartServer, validateAgent, createSession, injectPromptTemplate, executePrompt, abortSession, deleteSession, closeServer, } from "../src/opencode.js";
...
function makeMockClient(overrides = {}) {
    return {
        ...
        session: {
            create: async (_params) => ({ data: { id: "sess-123" } }),
            prompt: async (_params) => ({
                data: {
                    info: { id: "msg-1", sessionID: "sess-123", role: "assistant" },
                    parts: [{ type: "text", text: "mock response" }],
                },
            }),
            abort: async (_params) => ({ data: {} }),
            delete: async (_params) => ({ data: {} }),
        },
        ...overrides,
    };
}
function makeMockSdkDeps(client) {
    return {
...
describe("opencode.connectOrStartServer - attach fail then spawn", () => {
    ...
    it("throws SPAWN_FAILED when both attach and spawn fail", async () => {
        ...
        await assert.rejects(() => connectOrStartServer(MOCK_CONFIG, { sdk }), (err) => err instanceof AppError && err.code === "SPAWN_FAILED");
    });
});
describe("opencode.createSession", () => {
    ...
    it("throws SESSION_CREATE_FAILED when no id returned", async () => {
        ...
        await assert.rejects(() => createSession(client, "test"), (err) => err instanceof AppError && err.code === "SESSION_CREATE_FAILED");
    });
});
describe("opencode.validateAgent", () => {
    ...
    it("throws CONFIG_INVALID when agent not found", async () => {
        ...
        await assert.rejects(() => validateAgent(client, "nonexistent"), (err) => err instanceof AppError &&
...
describe("opencode.abortSession", () => {
    it("calls session.abort", async () => {
        ...
        const client = makeMockClient({
            session: {
                abort: async (_p) => { aborted = true; return { data: {} }; },
            },
        });
    ...
    it("throws SESSION_ABORT_FAILED on error", async () => {
        ...
        await assert.rejects(() => abortSession(client, "sess-1"), (err) => err instanceof AppError && err.code === "SESSION_ABORT_FAILED");
    });
});
describe("opencode.deleteSession", () => {
    it("calls session.delete", async () => {
        ...
        const client = makeMockClient({
            session: {
                delete: async (_p) => { deleted = true; return { data: {} }; },
            },
        });
    ...
    it("throws SESSION_DELETE_FAILED on error", async () => {
        ...
        await assert.rejects(() => deleteSession(client, "sess-1"), (err) => err instanceof AppError && err.code === "SESSION_DELETE_FAILED");
    });
});
