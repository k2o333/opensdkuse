...
describe("main - agent validation wiring", () => {
    it("--agent flag is accepted and does not crash at parse/config stage", async () => {
        ...
        try {
            ...
            assert.ok(exitCode === 0 || exitCode === 1 || exitCode === 2 || exitCode === 3, `Expected valid exit code, got ${exitCode}`);
        }
        finally {
            try {
                unlinkSync(promptPath);
...
describe("main - signal handler stability", () => {
    it("multiple runs do not accumulate signal listeners", async () => {
        ...
        try {
            ...
            assert.equal(finalSigintCount, initialSigintCount, `SIGINT listeners grew from ${initialSigintCount} to ${finalSigintCount}`);
            assert.equal(finalSigtermCount, initialSigtermCount, `SIGTERM listeners grew from ${initialSigtermCount} to ${finalSigtermCount}`);
        }
        finally {
            try {
                unlinkSync(promptPath);
    ...
    it("SIGINT handler uses named function (can be removed)", async () => {
        ...
        try {
            ...
            assert.equal(afterCount, beforeCount, `SIGINT listener not properly removed (before=${beforeCount}, after=${afterCount})`);
        }
        finally {
            try {
                unlinkSync(promptPath);
    ...
    it("SIGTERM handler uses named function (can be removed)", async () => {
        ...
        try {
            ...
            assert.equal(afterCount, beforeCount, `SIGTERM listener not properly removed (before=${beforeCount}, after=${afterCount})`);
        }
        finally {
            try {
                unlinkSync(promptPath);
...
describe("main - keep-session behavior", () => {
    it("--keep-session flag is accepted and does not crash", async () => {
        ...
        try {
            ...
            assert.ok(exitCode === 0 || exitCode === 1 || exitCode === 3, `Expected valid exit code (0/1/3), got ${exitCode}`);
        }
        finally {
            try {
                unlinkSync(promptPath);
...
describe("main - structured output schema validation", () => {
    ...
    it("returns exit code 2 when --schema-file contains non-object JSON", async () => {
        ...
        try {
            ...
            try {
                unlinkSync(nonObjectSchemaPath);
            }
            catch { }
        }
    });
