...
describe("prompt.loadPromptTemplate", () => {
    ...
    after(() => {
        try {
            unlinkSync(join(TEST_DIR, "valid.md"));
        }
        catch { }
        try {
            unlinkSync(join(TEST_DIR, "empty.md"));
        }
        catch { }
        try {
            unlinkSync(join(TEST_DIR, "whitespace.md"));
        }
        catch { }
    });
    it("throws PROMPT_FILE_NOT_FOUND for missing file", () => {
        assert.throws(() => loadPromptTemplate(join(TEST_DIR, "nonexistent.md")), (err) => err instanceof AppError && err.code === "PROMPT_FILE_NOT_FOUND");
    });
    it("throws PROMPT_FILE_EMPTY for empty file", () => {
        const emptyPath = join(TEST_DIR, "empty.md");
        writeFileSync(emptyPath, "");
        assert.throws(() => loadPromptTemplate(emptyPath), (err) => err instanceof AppError && err.code === "PROMPT_FILE_EMPTY");
    });
    it("throws PROMPT_FILE_WHITESPACE_ONLY for whitespace-only file", () => {
        ...
        assert.throws(() => loadPromptTemplate(wsPath), (err) => err instanceof AppError && err.code === "PROMPT_FILE_WHITESPACE_ONLY");
    });
...
describe("prompt.validatePromptTemplate", () => {
    ...
    it("rejects empty content", () => {
        assert.throws(() => validatePromptTemplate("", 1000), (err) => err instanceof AppError && err.code === "PROMPT_FILE_EMPTY");
    });
    it("rejects whitespace-only content", () => {
        assert.throws(() => validatePromptTemplate("   \n  ", 1000), (err) => err instanceof AppError && err.code === "PROMPT_FILE_WHITESPACE_ONLY");
    });
    it("rejects too-long content", () => {
        const long = "a".repeat(1001);
        assert.throws(() => validatePromptTemplate(long, 1000), (err) => err instanceof AppError && err.code === "PROMPT_FILE_TOO_LONG");
    });
});
